...
// Helper for consistent auth errors
...
// Cookie options helper – production-ready for Render cross-subdomain
const getCookieOptions = (isClear = false) => {
  const isProd = process.env.NODE_ENV === 'production';
  return {
    httpOnly: true,
    secure: isProd,                         // MUST be true in prod (HTTPS)
    sameSite: isProd ? 'none' : 'lax',      // 'none' + secure for cross-site
    ...(isClear
      ? { expires: new Date(Date.now()) }
      : { maxAge: 60 * 60 * 1000 }          // 1 hour
    ),
    // domain: isProd ? '.onrender.com' : undefined  // optional – try without first
  };
};
