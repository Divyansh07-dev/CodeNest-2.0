...
const updateProblem = async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Check if the problem exists
    const DsaProblem = await Problem.findById(id);
    if (!DsaProblem) {
      return res.status(404).send("Problem ID not found");
    }

    // 3️⃣ Validate referenceSolution
    if (!referenceSolution || referenceSolution.length === 0) {
      return res.status(400).send("No reference solution provided");
    }

    // 4️⃣ Validate visibleTestCases
    if (!visibleTestCases || visibleTestCases.length === 0) {
      return res.status(400).send("No visible test cases provided");
    }

    // 5️⃣ Loop over reference solutions
    for (const { language, completeCode } of referenceSolution) {
      const languageId = getLanguageById(language);
      if (!languageId) {
        console.warn(`Invalid language: ${language}`);
        continue; // skip this reference solution
      }
      ...
      try {
        ...
        const failedTests = testResult.filter((t) => t.status_id !== 3);
        if (failedTests.length > 0) {
          console.warn("Some test cases failed for reference solution:", language);
          // Optionally, you can continue instead of returning
          // continue;
          return res
            .status(400)
            .send("Reference solution failed some visible test cases");
        }
      } catch (err) {
        console.error("Judge0 submission error:", err);
        return res.status(500).send("Judge0 submission error");
      }
    }

    // 6️⃣ Update the problem safely
