...
const submitCode = async (req,res)=>{
    ...
    try{
        ...
        const submitResult = await submitBatch(submissions); // You are sending code to Judge0
        const resultToken = submitResult.map((value)=> value.token); // Judge0 gives back tokens
        const testResult = await submitToken(resultToken); // You use tokens to get execution results
        ...
        // 5. Update User Solved Liststore that problem’s ID in the user’s solved list
...
// ⭐ NEW FUNCTION: Aggregates all of the logged-in user's submissions by day,
// for the streak heatmap. Counts EVERY submission (any status) as activity
// for that day — mirrors how LeetCode counts any submit attempt, not just accepted ones.
const getUserActivity = async (req, res) => {
    try {
        ...
        // Reshape into { "2026-07-04": 3, "2026-07-03": 1, ... } for the frontend
        const activityMap = {};
        activity.forEach(item => {
            activityMap[item._id] = item.count;
        });

        return res.status(200).json(activityMap);

    } catch (err) {
        console.error("Error fetching user activity:", err);
        return res.status(500).send("Internal Server Error while fetching activity.");
    }
}

module.exports = {submitCode,runCode, getSubmissionHistory, getUserActivity};